"""
Implementation of the Book of Ecclesiastes extractor.
"""
...
from app.preprocessing.bible.base.core import BaseBibleParser, PatternConfig, VERSE_TO_IDX, ALL_VERSES_SEQ
from app.preprocessing.bible.ecclesiastes.core import EcclesiastesExtractor as EcclesiastesExtractorBase

_UPPER = r"[\"'A-Z\u00C0-\u017F]"

class EcclesiastesExtractor(BaseBibleParser, EcclesiastesExtractorBase):
    """
    Extractor to parse, clean, and align the Book of Ecclesiastes.
    """

    def __init__(self):
        EcclesiastesExtractorBase.__init__(self)
    ...
    def _get_clamped_page_range(
        ...
        last_ch = max(self.CHAPTER_VERSES_MAP.keys())
        last_v = self.CHAPTER_VERSES_MAP[last_ch]
        start_ref_clamped = (
            start_ref if start_ref[0] == target_book else (target_book, 1, 1)
        )
        end_ref_clamped = (
            end_ref if end_ref[0] == target_book else (target_book, last_ch, last_v)
        )
    ...
    def extract_and_align(
        ...
        print(f"Extraction Validation for Ecclesiastes:")
