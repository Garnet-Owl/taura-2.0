"""
Implementation of the Book of Mark (Mariko) extractor.
"""
...
from app.preprocessing.bible.mark.core import MarkExtractor as MarkExtractorBase


class MarkExtractor(BaseBibleParser, MarkExtractorBase):
    """
    Extractor to parse, clean, and align the Book of Mark.
    """

    def __init__(self):
        MarkExtractorBase.__init__(self)
    ...
    def _get_clamped_page_range(
        ...
        end_ref_clamped = (
            end_ref if end_ref[0] == target_book else (target_book, 16, 20)
        )
    ...
    def _locate_verse_one(
        ...
        pattern_one = re.compile(r"(?<!\d)1(?!\d)\s*[\"'‘’“”A-ZĨŨÀ-ÞĀ-ſ]")
    ...
    def extract_and_align(
        ...
        print("Extraction Validation for Mark:")
