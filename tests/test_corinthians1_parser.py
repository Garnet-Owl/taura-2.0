"""Unit tests for the Book of 1 Corinthians parser/extractor."""
...
class TestCorinthians1Parser(unittest.TestCase):
    def test_corinthians1_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = Corinthians1Extractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("1 Akorinitho")))
            assert_that(english_name, is_(equal_to("1 Corinthians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1368)))
            assert_that(kik_end, is_(equal_to(1383)))
            assert_that(eng_start, is_(equal_to(1450)))
            assert_that(eng_end, is_(equal_to(1464)))

        with then("total verse count is 437"):
            assert_that(extractor.total_verses, is_(equal_to(437)))

    def test_clean_english_verse(self):
        """Should strip cross-reference markers from English verse text."""
        with given([]) as _:
            extractor = Corinthians1Extractor()
            raw_text = "Christ did not send me to baptize* 1:17, but to preach."
        ...
        with then("cross-reference marker is removed"):
            assert_that(cleaned, not_(contains_string("* 1:17")))
            assert_that(cleaned, contains_string("Christ did not send me"))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1450 1 Corinthians 1:1")))
            assert_that(body_text, contains_string("Paul, called to be an apostle"))
            assert_that(body_text, not_(contains_string("footnote ref")))
            assert_that(body_text, not_(contains_string("page 1")))

    def test_all_corinthians1_verses_aligned(self):
        """All 437 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = Corinthians1Extractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning 1 Corinthians"):
