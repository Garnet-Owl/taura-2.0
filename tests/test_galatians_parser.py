"""Unit tests for the Book of Galatians parser/extractor."""
...
class TestGalatiansParser(unittest.TestCase):
    def test_galatians_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = GalatiansExtractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("Agalatia")))
            assert_that(english_name, is_(equal_to("Galatians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1395)))
            assert_that(kik_end, is_(equal_to(1400)))
            assert_that(eng_start, is_(equal_to(1475)))
            assert_that(eng_end, is_(equal_to(1480)))

        with then("total verse count is 149"):
            assert_that(extractor.total_verses, is_(equal_to(149)))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1475 Galatians 1:1")))
    ...
    def test_all_galatians_verses_aligned(self):
        """All 149 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = GalatiansExtractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning Galatians"):
