"""Unit tests for the Book of 1 Thessalonians parser/extractor."""
...
class TestThessalonians1Parser(unittest.TestCase):
    def test_thessalonians1_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = Thessalonians1Extractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("1 Athesalonike")))
            assert_that(english_name, is_(equal_to("1 Thessalonians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1416)))
            assert_that(kik_end, is_(equal_to(1419)))
            assert_that(eng_start, is_(equal_to(1495)))
            assert_that(eng_end, is_(equal_to(1497)))

        with then("total verse count is 89"):
            assert_that(extractor.total_verses, is_(equal_to(89)))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1495 1 Thessalonians 1:1")))
    ...
    def test_all_thessalonians1_verses_aligned(self):
        """All 89 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = Thessalonians1Extractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning 1 Thessalonians"):
