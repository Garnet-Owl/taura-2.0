"""Unit tests for the Book of Acts parser/extractor."""
...
class TestActsParser(unittest.TestCase):
    def test_acts_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = ActsExtractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("Atũmwo")))
            assert_that(english_name, is_(equal_to("Acts")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1312)))
            assert_that(kik_end, is_(equal_to(1349)))
            assert_that(eng_start, is_(equal_to(1395)))
            assert_that(eng_end, is_(equal_to(1433)))

        with then("total verse count is 1007"):
            assert_that(extractor.total_verses, is_(equal_to(1007)))

    def test_clean_english_verse(self):
        """Should strip cross-reference markers from English verse text."""
        with given([]) as _:
            extractor = ActsExtractor()
            raw_text = "And it shall come to pass* 2:17 that I will pour out my Spirit."
        ...
        with then("cross-reference marker is removed"):
            assert_that(cleaned, not_(contains_string("* 2:17")))
            assert_that(cleaned, contains_string("And it shall come to pass"))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
                ...
                "blocks": [
                    {
                        "lines": [
                            ...
                            },
                            {
                                ...
                                "spans": [
                                    {
                                        "text": "In my former book, Theophilus",
                                        "font": "Normal",
                                        "size": 12.0,
                                    }
                                ],
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1395 Acts 1:1")))
            assert_that(body_text, contains_string("In my former book, Theophilus"))
            assert_that(body_text, not_(contains_string("footnote ref")))
            assert_that(body_text, not_(contains_string("page 1")))

    def test_all_acts_verses_aligned(self):
        """All 1007 Acts verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = ActsExtractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning Acts"):
