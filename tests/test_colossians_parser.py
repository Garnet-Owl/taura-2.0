"""Unit tests for the Book of Colossians parser/extractor."""
...
class TestColossiansParser(unittest.TestCase):
    def test_colossians_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = ColossiansExtractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("Akolosai")))
            assert_that(english_name, is_(equal_to("Colossians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1412)))
            assert_that(kik_end, is_(equal_to(1415)))
            assert_that(eng_start, is_(equal_to(1491)))
            assert_that(eng_end, is_(equal_to(1494)))

        with then("total verse count is 95"):
            assert_that(extractor.total_verses, is_(equal_to(95)))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1491 Colossians 1:1")))
    ...
    def test_all_colossians_verses_aligned(self):
        """All 95 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = ColossiansExtractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning Colossians"):
