"""Unit tests for the Book of 2 Corinthians parser/extractor."""
...
class TestCorinthians2Parser(unittest.TestCase):
    def test_corinthians2_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = Corinthians2Extractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("2 Akorinitho")))
            assert_that(english_name, is_(equal_to("2 Corinthians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1384)))
            assert_that(kik_end, is_(equal_to(1394)))
            assert_that(eng_start, is_(equal_to(1465)))
            assert_that(eng_end, is_(equal_to(1474)))

        with then("total verse count is 257"):
            assert_that(extractor.total_verses, is_(equal_to(257)))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1465 2 Corinthians 1:1")))
    ...
    def test_all_corinthians2_verses_aligned(self):
        """All 257 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = Corinthians2Extractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning 2 Corinthians"):
