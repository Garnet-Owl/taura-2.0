"""Unit tests for the Book of Genesis parser/extractor."""
...
class TestGenesisParser(unittest.TestCase):
    def test_genesis_extractor_constants(self):
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("Kĩambĩrĩria")))
            assert_that(english_name, is_(equal_to("Genesis")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(4)))
            assert_that(kik_end, is_(equal_to(62)))
            assert_that(eng_start, is_(equal_to(6)))
            assert_that(eng_end, is_(equal_to(61)))
