"""Unit tests for the Book of Philippians parser/extractor."""
...
class TestPhilippiansParser(unittest.TestCase):
    def test_philippians_extractor_constants(self):
        """Should initialize with correct book names, page constants, and verse total."""
        with given([]) as _:
            extractor = PhilippiansExtractor()
        ...
        with then("names are correct"):
            assert_that(kikuyu_name, is_(equal_to("Afilipi")))
            assert_that(english_name, is_(equal_to("Philippians")))
        ...
        with then("pages are correct"):
            assert_that(kik_start, is_(equal_to(1407)))
            assert_that(kik_end, is_(equal_to(1411)))
            assert_that(eng_start, is_(equal_to(1487)))
            assert_that(eng_end, is_(equal_to(1490)))

        with then("total verse count is 104"):
            assert_that(extractor.total_verses, is_(equal_to(104)))
    ...
    def test_extract_page_body_text_filters_headers_and_footnotes(self):
        ...
        with then("only body text survives"):
            assert_that(body_text, not_(contains_string("1487 Philippians 1:1")))
    ...
    def test_all_philippians_verses_aligned(self):
        """All 104 verse pairs must be extracted with no missing or empty on either side."""
        with given([]) as _:
            extractor = PhilippiansExtractor()
            kik_pdf = "data/Kikuyu_Bible_all.pdf"
            eng_pdf = "data/English_Bible_all.pdf"

        with when("extracting and aligning Philippians"):
